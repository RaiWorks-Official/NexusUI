...
function Library:CreateWindow(config)
	...
	function Window:Tab(name, index)
		...
		TabButton.BackgroundTransparency = 1  -- Start with transparent background
		TabButton.BorderSizePixel = 0
		TabButton.Text = name
		TabButton.TextColor3 = currentTheme.SubText  -- Start with SubText color (medium gray)
		...
		-- Removed background hover effect, only text color changes
		TabButton.MouseEnter:Connect(function()
			if Window.CurrentTab ~= Tab then
				-- Only brighten text color slightly on hover
				Tween(TabButton, {TextColor3 = currentTheme.Text}, 0.25)
			end
		end)
		
		TabButton.MouseLeave:Connect(function()
			if Window.CurrentTab ~= Tab then
				-- Return to SubText color when not selected
				Tween(TabButton, {TextColor3 = currentTheme.SubText}, 0.25)
			end
		end)
		...
		TabButton.MouseButton1Click:Connect(function()
			for _, tab in pairs(Window.Tabs) do
				...
				-- Reset all tabs to SubText color (medium gray) with smooth tween
				Tween(tab.Button, {TextColor3 = currentTheme.SubText}, 0.3)
			end
			TabContent.Visible = true
			-- Selected tab gets white text color with smooth tween
			Tween(TabButton, {TextColor3 = currentTheme.Text}, 0.3)
		...
		if not Window.CurrentTab then
			TabContent.Visible = true
			TabButton.TextColor3 = currentTheme.Text  -- First tab is white
			Window.CurrentTab = Tab
		end
